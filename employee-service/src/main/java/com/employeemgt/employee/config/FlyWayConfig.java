...
public class FlyWayConfig {
    ...
    private void createDatabaseIfNotExists() {
        ...
        String postgresUrl = url.replace("/employee_service_db", "/postgres");
        
        try (Connection connection = DriverManager.getConnection(postgresUrl, username, password);
             Statement statement = connection.createStatement()) {
            
            statement.executeUpdate("CREATE DATABASE employee_service_db");
            System.out.println("Database 'employee_service_db' created successfully");
            
        } catch (SQLException e) {
            if (e.getSQLState().equals("42P04")) { // Database already exists
                System.out.println("Database 'employee_service_db' already exists");
            } else {
